...
class _WardenDashboardState extends State<WardenDashboard> {
  ...
  // ---------------------------------------------------------------------------
  // DETAILED STUDENT INSPECTION MODAL (WITH VIOLATIONS & LIVE HISTORY)
  // ---------------------------------------------------------------------------
  ...
  List<dynamic> _getOutNow(List<dynamic> requests) =>
      requests.where((r) => r['status'] == 'approved' || r['status'] == 'exited').toList();

  List<dynamic> _getLateReturns(List<dynamic> requests) {
    final now = DateTime.now();
    return requests.where((r) {
      if (r['status'] != 'approved' && r['status'] != 'exited') return false;
      if (r['expected_return_time'] == null) return false;
      return now.isAfter(DateTime.parse(r['expected_return_time']));
    }).toList();
  }

  void _showFilteredList(String title, List<dynamic> listData, Color accentColor) {
    showModalBottomSheet(
      context: context,
      shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(24))),
      builder: (context) {
              ...
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    final req = listData[index];
                    return Card(
                      child: ListTile(
                        leading: Icon(Icons.person, color: accentColor),
                        title: Text("Pass ID: ${req['id'].toString().substring(0, 8)}...", style: const TextStyle(fontWeight: FontWeight.bold)),
                ...
                return StreamBuilder<List<Map<String, dynamic>>>(
                  ...
                  builder: (context, studentsSnapshot) {
                    ...
                    return IndexedStack(
                      ...
                      children: [
                        _buildAnalysisHomeTab(requests),
  ...
  // ---------------------------------------------------------------------------
  Widget _buildAnalysisHomeTab(List<dynamic> requests) {
    ...
    final outNowList = _getOutNow(requests);
      ...
      children: [
        ...
        GridView.count(
          ...
          children: [
            ...
            _buildInteractiveCard(outNowList.length.toString(), "Out Now", Icons.people_outline, Colors.blue, () => _showFilteredList("Students Out Now", outNowList, Colors.blue)),
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return Card(
          ...
          child: ListTile(
            onTap: () => _showStudentDetailsModal(student), // TAP NAME TO SEE DETAILS
            ...
            subtitle: Text("Reg No: ${student['registration_number'] ?? 'N/A'} | Room: ${student['room_details'] ?? 'N/A'}"),
            trailing: Container(
              padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
              decoration: BoxDecoration(
